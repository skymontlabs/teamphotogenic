...
class RateLimiter {
    int max_requests_; // Maximum number of requests per client within the duration.
    std::chrono::seconds duration_; // Time window for the rate limit.
    size_t capacity_; // Maximum number of clients to track for rate limiting.
    std::list<std::string> lru_list_; // Least Recently Used (LRU) list of client IDs.
