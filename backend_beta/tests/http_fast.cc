...
private:
    ...
    beast::flat_static_buffer<1 << 21> buffer_;
    ...
    void process_request(http::request<request_body_t, http::basic_fields<alloc_t>> const& req)
    {
        ...
        case http::verb::post:
            read_file(req.target(), req.body());
            send_text_response(
                http::status::ok,
                "Uploaded file'" + std::string(req.target()) + "'\r\n");
            break;
    ...
    void read_file(std::string_view target, std::string_view sv)
    {
        std::ofstream file("." + std::string(target), std::ios::binary); // Opening file in binary mode

        if (file.is_open()) {
            // Write the contents of the string_view to the file
            file.write(sv.data(), sv.size());
            file.close();
        } else {
