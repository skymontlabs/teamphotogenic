...
class rconnection : public boost::asio::coroutine
{
...
public:
    rconnection(boost::asio::io_service& is):
    io_service_(is) {}
...
#include <boost/asio/yield.hpp>
    void get_stackless(std::string key, boost::system::error_code ec = {}, std::size_t length = 0) {
        reenter (this) {
            // Asynchronously write the command to the server
            yield redis_connection_->async_write(tx_buff, bredis::single_command_t("GET", key.data()),
            [this, &key](boost::system::error_code error_code, std::size_t consumed) {
                if (error_code) {
                ...
                this->get_stackless(key, error_code, 0);
            });
